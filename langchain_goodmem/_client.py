...
class GoodMemClient:
    ...
    def get_space(self, *, space_id: str) -> dict[str, Any]:
        ...
        Returns:
            A dictionary with the space metadata under the `space` key.
        ...
        return {"success": True, "space": body}
    ...
    def update_space(
        ...
        *,
        ...
        Only fields explicitly provided are sent in the request body.
        `replace_labels` and `merge_labels` are mutually exclusive on the
        server, so callers should pass at most one.

        Args:
            space_id: The UUID of the space to update.
            name: New space name.
            public_read: Whether the space is publicly readable.
            replace_labels: Replace the entire label set with this map.
            merge_labels: Merge these labels into the existing label set.

        Returns:
            A dictionary with the updated space and a confirmation message.
    ...
    def list_memories(
        ...
        *,
        ...
        """List memories in a space, with optional filters and pagination.

        Args:
            space_id: The UUID of the space.
            max_results: Maximum number of memories to return per page.
            next_token: Opaque pagination cursor returned from a prior call.
            status_filter: Restrict to a specific processing status
                (`PENDING`, `PROCESSING`, `COMPLETED`, or `FAILED`).
            include_content: Whether to inline the original content for each
                memory.
            filter_expression: Server-side metadata filter expression.

        Returns:
            A dictionary with `memories`, `totalMemories`, and `nextToken`.
    ...
    def retrieve_memories(
        ...
        *,
        ...
        enabled and no results are found initially. Optional post-processor
        knobs enable server-side reranking, relevance filtering, chronological
        re-sort, and LLM-generated summary (`abstractReply`).
        ...
        Args:
            ...
            reranker_id: UUID of a reranker model to reorder results by direct
                query-chunk scoring.
            llm_id: UUID of an LLM that generates the `abstractReply` summary.
            relevance_threshold: Drop results below this score (0..1).
            llm_temperature: LLM creativity (0..2). Translated to the wire key
                `llm_temp`.
            chronological_resort: Re-sort surviving results by creation time.
            post_processor_max_results: Override the post-processor cap; if
                omitted the server uses the top-level `requestedSize`.

        Returns:
            A dictionary containing matched results, memory definitions, and
            optionally an `abstractReply`.
        ...
        if reranker_id is not None:
            post_processor_config["reranker_id"] = reranker_id
        if llm_id is not None:
            post_processor_config["llm_id"] = llm_id
        if relevance_threshold is not None:
            post_processor_config["relevance_threshold"] = relevance_threshold
        if llm_temperature is not None:
            post_processor_config["llm_temp"] = llm_temperature
        if chronological_resort is not None:
            post_processor_config["chronological_resort"] = chronological_resort
        if post_processor_max_results is not None:
            post_processor_config["max_results"] = post_processor_max_results
